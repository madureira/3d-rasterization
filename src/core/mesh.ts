...
function toPoint([x, y, z]: [number, number, number]): Vec3 {
  return new Vec3(x, y, z);
}

function toPolygon(shape: Array<any>) {
  return shape.map(toPoint);
}

function offset(point: Vec3, position: Vec3): void {
  point.x += position.x;
  point.y += position.y;
  point.z += position.z;
}

function rotate(point: Vec3, rotation: Vec3): void {
  const sin = new Vec3(
    Math.sin(rotation.x),
    Math.sin(rotation.y),
    Math.sin(rotation.z)
...
class Mesh {
  ...
  constructor(polygons: Array<any>) {
...
export function createMesh(model: Array<any>) {
